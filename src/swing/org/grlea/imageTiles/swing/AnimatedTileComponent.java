...
// $Id: AnimatedTileComponent.java,v 1.1 2004-09-04 07:59:37 grlea Exp $
...
/**
 * <p>A {@link JComponent} that renders an Image Tiles Pipeline onto itself.</p>
 *
 ...
 * <p>Usage:<pre>
 *    Pipeline pipeline = ...;
 *    AnimatedTileComponent component = new AnimatedTileComponent(pipeline, true);
 *
 *    Container container = ...;
...
extends JComponent
{
   ...
   /**
    * Creates a new <code>AnimatedTileComponent</code> that will render the given pipeline.
    *
    * @param pipeline the image tiles pipeline to render
    *
    * @param autoStart <code>true</code> if the component should automatically start the pipeline
    * when the component is first shown, <code>false</code> if starting of the pipline will be
    * performed manually (i.e. outside this class).
    */
   public
   AnimatedTileComponent(final Pipeline pipeline, boolean autoStart)
   {
      if (pipeline == null)
         throw new IllegalArgumentException("pipeline cannot be null.");

      this.pipeline = pipeline;
      renderer = new ComponentPipelineRenderer();
      pipeline.addFrameListener(new ComponentRepaintPipelineListener(this));
   ...
   {
      renderer.render(pipeline, this, (Graphics2D) graphics);
   }
}